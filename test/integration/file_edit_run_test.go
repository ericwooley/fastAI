...
func TestWorkspaceEditingToolOrchestration(t *testing.T) {
	...
	blockedServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/chat/completions":
			...
			body, err := io.ReadAll(r.Body)
			if err != nil {
				t.Fatalf("read blocked body: %v", err)
			}
			var payload struct {
				Messages []struct {
					Role       string `json:"role"`
			...
			toolMsg := payload.Messages[len(payload.Messages)-1]
			content, ok := toolMsg.Content.(string)
			if toolMsg.Role != "tool" || toolMsg.ToolCallID != "write-2" || !ok || !strings.Contains(content, `"status":"blocked"`) || !strings.Contains(content, "repository boundary") {
				t.Fatalf("unexpected blocked tool message: %+v", toolMsg)
			}
	...
	result, err = blockedRunner.Run(context.Background(), agent.Request{Prompt: "Try to write outside the repo", Model: "github:gpt-4.1", SessionID: "s", AccessToken: "token"})
	if err != nil {
		t.Fatalf("blocked run should continue after tool result: %v", err)
	}
	if result.Summary != "blocked" || len(result.FileChanges) != 1 || result.FileChanges[0].Status != "blocked" {
		t.Fatalf("unexpected blocked result: %+v", result)
	}
}

func TestBlockedCommandToolResultReturnsToModel(t *testing.T) {
	t.Parallel()
	repo := tempRepo(t)
	callCount := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		if callCount == 1 {
			_, _ = w.Write([]byte(`{"choices":[{"message":{"role":"assistant","tool_calls":[{"id":"cmd-1","type":"function","function":{"name":"run_command","arguments":"{\"command\":\"ls -la\",\"working_directory\":\"/\"}"}}]}}]}`))
			return
		}
		...
		toolMsg := payload.Messages[len(payload.Messages)-1]
		content, ok := toolMsg.Content.(string)
		if toolMsg.Role != "tool" || toolMsg.ToolCallID != "cmd-1" || !ok || !strings.Contains(content, `"status":"blocked"`) || !strings.Contains(content, "command blocked by repository boundary") {
			t.Fatalf("unexpected command tool message: %+v", toolMsg)
		}
		_, _ = w.Write([]byte(`{"choices":[{"message":{"role":"assistant","content":"command was blocked and reported"}}]}`))
	}))
	defer server.Close()

	adapter := githubmodels.NewValidator(server.Client(), server.URL, "fastAI/test")
	runner := agent.NewLocalRunnerWithPromptRunner(workspace.NewEditor(repo), commandexec.NewExecutor(repo), adapter, adapter)
	result, err := runner.Run(context.Background(), agent.Request{Prompt: "Run ls outside the repo", Model: "github:gpt-4.1", SessionID: "s", AccessToken: "token"})
	if err != nil {
		t.Fatalf("run should continue after blocked command tool result: %v", err)
	}
	if result.Summary != "command was blocked and reported" || len(result.CommandResults) != 1 || result.CommandResults[0].Status != "blocked" {
