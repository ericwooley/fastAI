...
func TestCLIFileEditingAndBlockedPath(t *testing.T) {
	...
	code, out, errOut = execute(t, []string{"--model", "github:gpt-4.1", "create file note.txt with content hi"}, deps)
	...
	code, _, errOut = execute(t, []string{"--model", "github:gpt-4.1", "create file ../bad.txt with content no"}, deps)
