...
func TestCLISessionContinuationAndCommandFailure(t *testing.T) {
	...
	if code, _, errOut := execute(t, []string{"login", "copilot"}, deps); code != 0 {
		t.Fatalf("login code=%d err=%q", code, errOut)
	}
	code, out, errOut := execute(t, []string{"--provider", "github-copilot", "--model", "github:gpt-4.1", "--permissions", "all", "--session", "follow", "run command printf ok"}, deps)
	...
	code, out, errOut = execute(t, []string{"--provider", "github-copilot", "--model", "github:gpt-4.1", "--permissions", "all", "run command printf ok"}, deps)
	if code != 0 || out != "" || !strings.Contains(errOut, "command: printf ok exit=0") {
	...
	code, out, errOut = execute(t, []string{"--provider", "github-copilot", "--model", "github:gpt-4.1", "--permissions", "all", "--session", "follow", "run command exit 7"}, deps)
