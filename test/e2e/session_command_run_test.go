...
func TestCLISessionContinuationAndCommandFailure(t *testing.T) {
	...
	code, out, errOut := execute(t, []string{"--provider", "github-copilot", "--model", "github:gpt-4.1", "--session", "follow", "run command printf ok"}, deps)
	...
	code, out, errOut = execute(t, []string{"--provider", "github-copilot", "--model", "github:gpt-4.1", "run command printf ok"}, deps)
	if code != 0 || out != "" || !strings.Contains(errOut, "command: printf ok exit=0") {
	...
	code, out, errOut = execute(t, []string{"--provider", "github-copilot", "--model", "github:gpt-4.1", "--session", "follow", "run command exit 7"}, deps)
